...
credito_dataset_final <- to.factors(df = credito_dataset_scaled, 
...
formula.init <- "credit.rating ~ ." # a esquerda, a variável a ser prevista,
                                    # a direita, as variáveis explanatórias.
...
# the linear predictor and a description of 
# the error distribution.

modelo_v1 <- glm(formula = formula.init, 
                 data = train.data, 
                 family = "binomial")
...
# Confusion Matrix
confusionMatrix(table(data = previsoes, reference = test.class.var), 
                positive = '1')

# Feature Selection - Selação de Atributos
formula <- "credit.rating ~ ."
formula <- as.formula(formula)
           # cria um método para repetir um modelo várias vezes 
...
modelo_v2 <- glm(formula = formula.new, 
                 data = train.data, 
                 family = "binomial")
...
previsoes_new <- predict(modelo_v2, 
                         test.data, 
                         type = "response") 
previsoes_new <- round(previsoes_new)

# Confusion Matrix
confusionMatrix(table(data = previsoes_new, 
...
# Plot
par(mfrow = c(1, 2))
plot.roc.curve(previsoes_finais, 
               title.text = "Curva ROC")
...
new.categorical.vars <- c('account.balance', 
                          'previous.credit.payment.status', 
                          'credit.purpose', 
                          'savings')

# Aplica as transformações
novo_dataset_final <- to.factors(df = novo_dataset, 
                                 variables = new.categorical.vars)
str(novo_dataset_final)

novo_dataset_final <- scale.features(novo_dataset_final,
                                     new.numeric.vars)
str(novo_dataset_final)
...
# Previsões
?predict
previsao_novo_cliente <- predict(modelo_final, 
                                 newdata = novo_dataset_final, 
                                 type = "response")
